...
class App extends Component {
  ...
  onClickThumbnail = id => {
    ...
    this.setState({
      randomImg: imagesList[Math.ceil(Math.random() * imagesList.length - 1)],
    })
    if (randomImg.id === id) {
  ...
  render() {
    const {activeTab, score, seconds, isGameRunning, randomImg} = this.state
    const filteredList = this.getParticularTabItems()
      ...
      <div className="bg-match-game">
        ...
        {isGameRunning && (
          <div className="game-section">
            <ul className="top-photos">
              <li>
                <img className="img" src={randomImg.imageUrl} alt="match" />
              </li>
            </ul>
